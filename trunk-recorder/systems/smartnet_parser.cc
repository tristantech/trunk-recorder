...
if (x.size()<3) {
  ...
  if (is_chan(stack[0].cmd, system) && stack[0].grp && getfreq(stack[0].cmd, system)) {
    ...
    if (stack[1].cmd == 0x308) {
      // Analog voice grant
      //cout << "NEW ANALOG GRANT!! CMD1: " << fixed << hex << stack[1].cmd << " 0add: " << dec <<  stack[0].address << " 0full_add: " << stack[0].full_address  << " 1add: " << stack[1].address << " 1full_add: " << stack[1].full_address  << endl;
      message.message_type = GRANT;
      message.source = stack[1].full_address;
      message.mode = TRUNKCALLMODE_ANALOG;
    } else if (stack[1].cmd == 0x321) {
      // P25 voice grant
      //cout << "NEW P25 GRANT!! CMD1: " << fixed << hex << stack[1].cmd << " 0add: " << dec <<  stack[0].address << " 0full_add: " << stack[0].full_address  << " 1add: " << stack[1].address << " 1full_add: " << stack[1].full_address  << endl;
      message.message_type = GRANT;
      message.source = stack[1].full_address;
      message.mode = TRUNKCALLMODE_P25;
